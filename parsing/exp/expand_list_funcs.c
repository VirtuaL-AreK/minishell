...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/24 14:56:49 by aanmazir          #+#    #+#             */
/*   Updated: 2025/04/24 14:58:43 by aanmazir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_dollar(t_exp_ctx *ctx)
{
	if (ctx->in_sq || ctx->s[ctx->idx] != '$')
		return (0);
	if (!ctx->in_dq && (ctx->s[ctx->idx + 1] == '\'' || ctx->s[ctx->idx
				+ 1] == '"'))
	{
...
{
	if (ctx->in_dq && ctx->s[ctx->idx] == '\\' && (ctx->s[ctx->idx + 1] == '"'
...
int	handle_tilde(t_exp_ctx *ctx)
{
	...
	if (ctx->idx == 0 && ctx->s[ctx->idx] == '~' && (ctx->s[ctx->idx
				+ 1] == '\0' || ctx->s[ctx->idx + 1] == '/'))
	{
